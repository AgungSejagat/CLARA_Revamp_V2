//File-file ini berfungsi untuk menulis data  ke database
...
class DeviceRepository {
  ...
  Future<List<Device>> getDevices() async { //Untuk mengambil semua data device
  ...
  Future<void> addDevice(Device device) async { //Untuk nambah data device
    final db = await Sqlite().database;
    await db.insert(
      'user_devices',
      device.toJson(),
  ...
  Future<void> removeDevice(Device device) async { //Untuk menghapus data device
    final db = await Sqlite().database;
    await db.delete(
      'user_devices',
      where: 'espId = ?',
